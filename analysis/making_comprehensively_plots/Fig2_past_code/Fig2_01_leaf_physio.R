...
df.merge_traits<-df.merge_traits %>%
  mutate(Position=case_when(c(Position=="L") ~"Lower",
                            ...
                            c(Position=="U") ~"Upper"))
#save the data:
df.leaf_physio<-list("df.Amax"=df.merge_Amax,"df.A"=df.merge_A,
                     "df.gs_E"=df.merge_Gs_E,"df_FvFm_NPQ_qN"=df.merge_FvFm_NPQ_qN,
                     "df_Pigments"=df.merge_Pigments,"df_traits"=df.merge_traits)
save.path<-"D:/Github/Velux_project/data/Comprehensive_plot_data/Fig2/"
saveRDS(df.leaf_physio,file = paste0(save.path,"df.leaf_physio.RDS"))
#------------------
#(3)plotting
#------------------
#addding mean and sd
