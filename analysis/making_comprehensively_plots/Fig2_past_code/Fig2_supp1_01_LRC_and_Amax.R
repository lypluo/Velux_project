...
plot_LRC_fun<-function(df,df.Tha,df.Dav,CamN,xlab,ylab,legend){
  ...
  p_plot<-ggplot(mapping = aes(Qin, A)) +
    ...
    scale_color_manual("",values = c("DAV-L"="red4","DAV-U"="red1",
                                     "THA-L"="orange4","THA-U"="orange1"))+
    labs(
...
ggsave(p_Amax,filename = paste(save.path,"Fig2_supp1_Amax_determination.png"),
       width = 10,height = 8)
