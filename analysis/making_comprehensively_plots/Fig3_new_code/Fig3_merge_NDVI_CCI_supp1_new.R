...
load(paste0(load.path,"p_leaf_spectra_NDVI_new.RDA"))
load(paste0(load.path,"p_crown_NDVI_new.RDA"))
...
#-------
###A-->For NDVI
p_merge_image_NDVI <- p_merge_image_NDVI + theme(plot.margin = margin(5, 5, 5, 5))
p_NDVI_boxplot <- p_NDVI_boxplot + theme(plot.margin = margin(5, 5, 5, 5))
...
#
p_NDVI_Meteo <- p_NDVI_Meteo + theme(plot.margin = margin(5, 5, 5, 5))
p_crown_NDVI <- p_crown_NDVI + theme(plot.margin = margin(5, 5, 5, 5))
p_merge2<-plot_grid(p_NDVI_Meteo,
                    p_crown_NDVI,
                    labels = c("(a)","(b)"),
...
ggsave(paste0(save.path,"Fig3_supp1_NDVI_new.png"),
